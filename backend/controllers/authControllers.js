...
export const loginUser = async (req, res) => {
    ...
    try {
        const user = await User.findOne({
            email: email.toLowerCase().trim() // Match registration lowercase
        });
        if (!user) {
            return res.status(400).json({ success: false, message: "Invalid email or password." });
        }
        ...
        const token = jwt.sign(
            {
                id: user._id,
                email: user.email,
                firstName: user.firstName // Include in token
            },
            process.env.JWT_SECRET || "khavkbrvu84oih8i5h294h2f2yf8h2842hf",
            { expiresIn: "1h" }
        );

// Return response with same user structure as /users/me
        // In loginUser controller
        // In loginUser controller, add headers:

        // Modify the login response to remove manual CORS headers:
        res
            .cookie('token', token, {
                httpOnly: true,
